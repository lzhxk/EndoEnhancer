...
class Renderer(object):
    ...
    def eval_points(self,
        ...
        for ro, rd, zv, dst in zip(rays_o_split, rays_d_split, z_vals_split, dists_split):
            out = net(ro, rd, zv, dst, render_params=render_params, uncertainty_params=uncertainty_params, near_far=near_far)
            if len(render_out) == 0:
                render_out = out
                continue
    ...
    def render_batch_ray(self,
        ...
        if N_surface > 0:
            ...
            # print(f"snr shape: {snr.shape}", f"sfar shape: {sfar.shape}", f"valid_depth shape: {valid_depth.shape}", f"t_vals_surface shape: {t_vals_surface.shape}")
        ...
        rets = self.eval_points(rays_o, rays_d, z_vals, dists, net, render_params=render_params, uncertainty_params=uncertainty_params, near_far=near_far)
