...
class BaseDataset(Dataset):
    ...
    def depthloader(self, index):
        ...
        if '.png' in depth_path:
            ...
            depth_data = depth_data.astype(np.float32) / 255.0 * 5000.0
        elif '.jpg' in depth_path:
            # 读取为灰度图
            depth_data = cv2.imread(depth_path, cv2.IMREAD_GRAYSCALE)
            # 映射到最大深度5000
            depth_data = depth_data.astype(np.float32) / 255.0 * 5000.0
...
class ScanNet(BaseDataset):
    def __init__(self, cfg, args, device='cuda:0'):
        ...
        self.color_paths = sorted(glob.glob(os.path.join(
            self.input_folder, 'color', '*.jpg')), key=lambda x: int(os.path.basename(x)[-9:-4]))[:max_frames][::stride] #[:-4]
        self.depth_paths = None
        # self.depth_paths = sorted(glob.glob(os.path.join(
            # self.input_folder, 'depth', '*.png')), key=lambda x: int(os.path.basename(x)[-9:-4]))[:max_frames][::stride]
 
        self.n_img = len(self.color_paths)
        self.poses = None
        # self.load_poses(os.path.join(
        #     self.input_folder, 'poses.npz'))
    ...
    def load_poses(self, path):
        ...
        if os.path.exists(path):
            ...
            for i in range(len(poses['arr_0'])):
                T_i = np.append(poses['arr_0'][i], [[0.0,0.0,0.0,1.0]], axis=0)
                T_i = np.linalg.inv(T_i)
                # T_i = torch.from_numpy(T_i).float()
                c2w = self.poses[-1] @ T_i
                self.poses.append(c2w)
        else:
            for i in range(self.n_img):
                c2w = np.eye(4)
