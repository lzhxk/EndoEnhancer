...
class CostRegNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 确保conv7的输出尺寸与conv4匹配
        conv7_output = self.conv7(x)
        if conv7_output.shape != conv4.shape:
            # print(f"CostRegNet: conv7_output shape {conv7_output.shape} != conv4 shape {conv4.shape}, adjusting...")
            conv7_output = F.interpolate(conv7_output, size=conv4.shape[2:], mode='trilinear', align_corners=False)
        x = conv4 + conv7_output
        
        
        # 确保conv9的输出尺寸与conv2匹配
        conv9_output = self.conv9(x)
        if conv9_output.shape != conv2.shape:
            # print(f"CostRegNet: conv9_output shape {conv9_output.shape} != conv2 shape {conv2.shape}, adjusting...")
            conv9_output = F.interpolate(conv9_output, size=conv2.shape[2:], mode='trilinear', align_corners=False)
        x = conv2 + conv9_output
        
        
        # 确保conv11的输出尺寸与conv0匹配
        conv11_output = self.conv11(x)
        if conv11_output.shape != conv0.shape:
            # print(f"CostRegNet: conv11_output shape {conv11_output.shape} != conv0 shape {conv0.shape}, adjusting...")
            conv11_output = F.interpolate(conv11_output, size=conv0.shape[2:], mode='trilinear', align_corners=False)
        cost = conv0 + conv11_output
        prob = self.prob(cost)
        return cost, prob
