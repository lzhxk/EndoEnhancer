...
from src.utils.utils import homo_warp
from inplace_abn import InPlaceABN
from .renderer import run_network_mvs
from .mvs_models import depth_regression, mvs_depth_regression, RefineNet, CascadeMVSNet
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
from functools import partial

def global_network_query_fn(pts, viewdirs, rays_feats, network_fn, embed_fn, embeddirs_fn, netchunk):
    return run_network_mvs(
        pts, viewdirs, rays_feats, network_fn,
        embed_fn=embed_fn,
        embeddirs_fn=embeddirs_fn,
        netchunk=netchunk
    )

def call_embedder(x, embedder_obj):
    return embedder_obj.embed(x)

def identity_fn(x):
    return x

def periodic_embed_fn(x, p_fn, freq):
    return p_fn(x * freq)
...
class Embedder:
    ...
    def create_embedding_fn(self):
        ...
        if self.kwargs['include_input']:
            ...
            embed_fns.append(identity_fn)  # 替换 lambda x: x
            out_dim += d
        ...
        for freq in freq_bands:
            for p_fn in self.kwargs['periodic_fns']:
                ...
                embed_fns.append(partial(periodic_embed_fn, p_fn=p_fn, freq=freq))
                out_dim += d
...
def get_embedder(multires, i=0, input_dims=3):
    ...
    embed = partial(call_embedder, embedder_obj=embedder_obj)
...
def create_ucnerf(cfg, pts_embedder=True, dir_embedder=True):
    """Instantiate mvs NeRF's MLP model.
    """

    if pts_embedder:
        embed_fn, input_ch = get_embedder(cfg['mapping']['model']['multires'], cfg['mapping']['model']['i_embed'])  # , input_dims=cfg['mapping']['model']['pts_dim'])
    else:
        embed_fn, input_ch = None, cfg['mapping']['model']['pts_dim']

    embeddirs_fn = None
    if dir_embedder:
        embeddirs_fn, input_ch_views = get_embedder(cfg['mapping']['model']['multires_views'], cfg['mapping']['model']['i_embed'])  # , input_dims=cfg['mapping']['model']['dir_dim'])
    else:
        embeddirs_fn, input_ch_views = None, cfg['mapping']['model']['dir_dim']

    skips = [4]
    model = UCNeRF(
        D=cfg['mapping']['model']['netdepth'],
        W=cfg['mapping']['model']['netwidth'],
        input_ch_pts=input_ch,
        skips=skips,
        input_ch_views=input_ch_views,
        input_ch_feat=cfg['mapping']['model']['feat_dim'],
        net_type=cfg['mapping']['model']['net_type'],
        view_num=cfg['mapping']['model']['view_num']
    ).to(device)

    grad_vars = []
    grad_vars += list(model.parameters())

    # network_query_fn = lambda pts, viewdirs, rays_feats, network_fn: run_network_mvs(
    #     pts, viewdirs, rays_feats, network_fn,
    #     embed_fn=embed_fn,
    #     embeddirs_fn=embeddirs_fn,
    #     netchunk=cfg['mapping']['model']['netchunk']
    # )
    network_query_fn = partial(
        global_network_query_fn,
        embed_fn=embed_fn,
        embeddirs_fn=embeddirs_fn,
        netchunk=cfg['mapping']['model']['netchunk']
    )

    if cfg['mapping']['model']['encode_a']:
        embedding_a = torch.nn.Embedding(cfg['mapping']['model']['N_vocab'], cfg['mapping']['model']['N_a'])
        grad_vars += list(embedding_a.parameters())

    EncodingNet = CascadeMVSNet(view_num=cfg['mapping']['model']['view_num'])
    ...
    if cfg['mapping']['model']['finetune'] is None:
        grad_vars += list(EncodingNet.parameters())

    start = 0
    ckpts = []
    if cfg['mapping']['model']['ckpt'] is not None and cfg['mapping']['model']['ckpt'] != 'None':
        ckpts = [cfg['mapping']['model']['ckpt']]
    ...
    render_kwargs_train = {
        ...
        'perturb': cfg['mapping']['model']['perturb'],
        'N_samples': cfg['mapping']['model']['N_samples'],
        'network_fn': model,
        'network_mvs': EncodingNet,
        'use_viewdirs': cfg['mapping']['model']['use_viewdirs'],
        'white_bkgd': cfg['mapping']['model']['white_bkgd'],
        'raw_noise_std': cfg['mapping']['model']['raw_noise_std']
    }
    if cfg['mapping']['model']['encode_a']:
