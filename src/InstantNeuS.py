...
import trimesh
import numpy as np
from .network.renderer import gen_pts_feats
from .utils.utils import get_ndc_coordinate
...
class ColorNetwork_adapt(nn.Module):
    def __init__(self,
                 d_in: int = 3,
                 d_feat: int = 32 - 1,
                 d_hidden=64,
                 n_layers=2,
                 device='cuda:0'):
        super(ColorNetwork_adapt, self).__init__()
        self.d_in = d_in
        self.d_feat = d_feat
        self.d_hidden = d_hidden
        self.n_layers = n_layers
        ...
        # 修改输入维度：embed_dim (view_pts编码) + 3 (pts_3d) + d_feat (input_feat)
        n_input_dims = embed_dim + 3 + d_feat
        ...
        with torch.cuda.device(device):
            ...
            # 修改输出维度：1 (sdf) + 3 (rgb)
            self.network = tcnn.Network(n_input_dims=n_input_dims, n_output_dims=4, network_config=network_config)

    def forward(self, pts_3d, input_feat):
        # 对pts_3d进行位置编码
        view_pts = torch.sin(pts_3d @ self._B.to(pts_3d.device))

        # 拼接输入：view_pts编码 + pts_3d + input_feat
        rendering_input = torch.cat([view_pts, pts_3d, input_feat], dim=1)

        x = self.network(rendering_input)

        # 分离输出：sdf和rgb
        out_sdf_adapt = x[:, 0:1]  # [n_pts, 1]
        out_rgb_adapt = torch.sigmoid(x[:, 1:])  # [n_pts, 3]

        return out_sdf_adapt, out_rgb_adapt
...
class InstantNeuS(nn.Module):
    def __init__(self, cfg, bound, device='cuda:0'):
        ...
        self.color_network_adapt = ColorNetwork_adapt(**cfg['color_network_adapt'], device=device)
        ...
        self.img_downscale = cfg['img_downscale']
        self.use_color_volume = cfg['use_color_volume']
    ...
    def get_training_parameters(self, ignore_keys=()):
        ...
        all_params = {
            ...
            'color_network_adapt': list(self.color_network_adapt.parameters()),
    ...
    def forward(self,
        ...
        # get input_feat
        # 假设 imgs_input: (1, V-1, 3, H, W)
        H, W = uncertainty_params['imgs_input'].shape[-2:]
        w2c_ref = uncertainty_params['pose_ref']['w2cs'][0]
        intrinsic_ref = uncertainty_params['pose_ref']['intrinsics'][0]
        outputs = uncertainty_params['outputs']
        inv_scale = torch.tensor([W-1, H-1], device=pts.device)
        # print(" uncertainty_params['near_fars']", uncertainty_params['near_fars'])
        # print("near_far",near_far)
        rays_ndc = get_ndc_coordinate(w2c_ref, intrinsic_ref, pts, inv_scale, uncertainty_params['near_fars'])
        input_feat = gen_pts_feats(uncertainty_params['imgs_input'], uncertainty_params['outputs'], pts, uncertainty_params['pose_ref'], rays_ndc, self.feat_dim, \
                               uncertainty_params['outputs']['stage3']['img_feats'], near_far, self.img_downscale, self.use_color_volume, self.net_type, confidence=uncertainty_params['uncertainty_map'])
        input_feat = input_feat.reshape(-1, self.feat_dim)
        ...
        def alpha_rgb_fn(pts_3d, dirs_3d, input_feat, dists_3d, mask):
            # [n_pts, 1], [n_pts, 3]
            ...
            out_sdf_adapt, out_rgb_adapt = self.color_network_adapt(pts_3d[mask], input_feat[mask])
            sdf_adapt = torch.ones([n_pts, 1]).to(device).to(out_sdf_adapt.dtype) * 100
            sdf_adapt[mask] = out_sdf_adapt
            rgb_adapt = torch.zeros([n_pts, 3]).to(device).to(out_rgb_adapt.dtype)
            rgb_adapt[mask] = out_rgb_adapt
            ...
            uncertainty = 1-input_feat[..., -1:]
            # rgb = torch.sigmoid(rgb_adapt*(1-uncertainty) + rgb*uncertainty)
            sdf = torch.relu(sdf_adapt*(1-uncertainty) + sdf*uncertainty)



            return sdf, rgb, alpha, gradients

        sdf, rgb, alpha, gradients = alpha_rgb_fn(pts, dirs, input_feat, dists, pts_mask) #input_feat
            ...
            for xi, xs in enumerate(X):
                for yi, ys in enumerate(Y):
                    for zi, zs in enumerate(Z):
                        ...
                        # print("self.realtime_bound",self.realtime_bound)
                        ...
                        # if len(np.nonzero(sdf-100)[0])!=0:
                        #     print("sdf",np.nonzero(sdf-100))
    ...
    def extract_geometry(self,
        ...
        # print("vertices, triangles",vertices, triangles)
