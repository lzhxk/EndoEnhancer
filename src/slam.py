...
from .network.mvs_models import CascadeMVSNet
...
class SLAM:
    def __init__(self, args, cfg):
        ...
        self.net = DroidNet()
        self.ConsistNet = CascadeMVSNet(view_num=cfg['mapping']['model']['view_num'])

        self.load_pretrained(cfg['tracking']['pretrained'])
        self.net.to(self.device).eval()
        self.net.share_memory()
        self.ConsistNet.to(self.device)
        self.ConsistNet.share_memory()
    ...
    def load_pretrained(self, pretrained):
        ...
        self.ConsistNet.load_state_dict(state_dict['model'], strict=True)
    ...
    def mapping(self, rank, dont_run=False):
        ...
        if not dont_run:
            ...
            for i in tqdm(range(self.post_processing_iters+3)):
                self.mapper(the_end=True, iter=i)
