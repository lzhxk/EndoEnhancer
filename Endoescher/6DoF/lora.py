...
class LoRALayer(nn.Module):
    def __init__(self, layer, r, lora_alpha, lora_dropout, merge_weights):
        super().__init__()
        self.layer = layer
        self.r = r
        ...
        if self.r > 0:
            self.lora_A = nn.Parameter(torch.zeros((self.layer.weight.shape[0], self.r)))
            self.lora_B = nn.Parameter(torch.zeros((self.r, self.layer.weight.shape[1]))) 
    ...
    def initial_weights(self):
        if self.r > 0:
            ...
            # if self.merge_weights and self.lora_alpha > 1:
            #     self.lora_A.data.mul_(self.lora_alpha / self.r)
    
    def forward(self, x):
        device = self.layer.weight.device
        
        if self.r > 0 and self.merge_weights:
            result = F.linear(x, (self.layer.weight + self.lora_A.to(device) @ self.lora_B.to(device) * self.scaling), bias=self.layer.bias)
            result = self.dropout(result)
            # print(x.shape, self.layer.weight.shape, (self.layer.weight + self.lora_A.to(device) @ self.lora_B.to(device) * self.scaling).shape,result.shape)
            # if self.lora_A.grad:
...
def load_lora(unet, lora_path=None):
    # 输出网络结构到文件
    with open("model_structure_lora.txt", "w") as f:
        print(unet, file=f)
    
    # change layer to LoRALayer
    assign_lora = partial(LoRALayer, r = 8, lora_alpha = 16, lora_dropout = 0, merge_weights = True)
    lora_q = True
    lora_k = True
    lora_v = False
    for layer in unet.down_blocks[:3]:
        for attn in layer.attentions:
            if lora_q:
                attn.transformer_blocks[0].attn1.to_q = assign_lora(attn.transformer_blocks[0].attn1.to_q)
                attn.transformer_blocks[0].attn2.to_q = assign_lora(attn.transformer_blocks[0].attn2.to_q)
            if lora_k:
                attn.transformer_blocks[0].attn1.to_k = assign_lora(attn.transformer_blocks[0].attn1.to_k)
                attn.transformer_blocks[0].attn2.to_k = assign_lora(attn.transformer_blocks[0].attn2.to_k)
            if lora_v:
                attn.transformer_blocks[0].attn1.to_v = assign_lora(attn.transformer_blocks[0].attn1.to_v)
                attn.transformer_blocks[0].attn2.to_v = assign_lora(attn.transformer_blocks[0].attn2.to_v)
    for layer in unet.up_blocks[1:]:
        for attn in layer.attentions:
            if lora_q:
                attn.transformer_blocks[0].attn1.to_q = assign_lora(attn.transformer_blocks[0].attn1.to_q)
                attn.transformer_blocks[0].attn2.to_q = assign_lora(attn.transformer_blocks[0].attn2.to_q)
            ...
            if lora_v:
                ...
                attn.transformer_blocks[0].attn2.to_v = assign_lora(attn.transformer_blocks[0].attn2.to_v)
    if lora_q:
        unet.mid_block.attentions[0].transformer_blocks[0].attn1.to_q = assign_lora(unet.mid_block.attentions[0].transformer_blocks[0].attn1.to_q)
        unet.mid_block.attentions[0].transformer_blocks[0].attn2.to_q = assign_lora(unet.mid_block.attentions[0].transformer_blocks[0].attn2.to_q)
    if lora_k:
        unet.mid_block.attentions[0].transformer_blocks[0].attn1.to_k = assign_lora(unet.mid_block.attentions[0].transformer_blocks[0].attn1.to_k)
        unet.mid_block.attentions[0].transformer_blocks[0].attn2.to_k = assign_lora(unet.mid_block.attentions[0].transformer_blocks[0].attn2.to_k)
    if lora_v:
        unet.mid_block.attentions[0].transformer_blocks[0].attn1.to_v = assign_lora(unet.mid_block.attentions[0].transformer_blocks[0].attn1.to_v)
        unet.mid_block.attentions[0].transformer_blocks[0].attn2.to_v = assign_lora(unet.mid_block.attentions[0].transformer_blocks[0].attn2.to_v)

    with open("model_structure_lora1.txt", "w") as f:
        print(unet, file=f)
        ...
        for valid_step, batch in tqdm(enumerate(validation_dataloader)):
            ...
            for _ in range(args.num_validation_images):
                with torch.autocast("cuda"):
                    # print("Generating image",input_image,input_image)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        image_encoder1 = CN_encoder.from_pretrained(args.pretrained_model_name_or_path, subfolder="image_encoder", revision=args.revision)
                        pipeline = Zero1to3StableDiffusionPipeline.from_pretrained(
                            args.pretrained_model_name_or_path,
                        ...
                        pipeline.image_encoder = image_encoder1
